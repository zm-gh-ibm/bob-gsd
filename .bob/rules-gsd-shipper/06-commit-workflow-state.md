...
write `.gsd/STATE.md`, `.gsd/VERIFY.md`, etc. as the phase progresses, and no mode commits
...
artifact — the Shipper commits it alongside the `.gsd/` state.
...
3. If any of those are staged, commit them as:
   ...
   ```
4. Then proceed to rule 01. With the `.gsd/` state and `AGENTS.md` committed, the only
   remaining tree contents are the phase's actual task commits — the clean-tree check now
   passes.

## Boundaries

- Stage the known GSD artifacts **only**: the `.gsd/` paths above and `AGENTS.md`. If
