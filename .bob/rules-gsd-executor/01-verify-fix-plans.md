...
whether `.gsd/VERIFY.md` contains an open fix plan from a failed verification cycle.
...
- After completing all fix items, continue with any remaining tasks from
  `.gsd/CONTEXT.md` in normal numeric order.
- Do NOT modify `.gsd/VERIFY.md` — only the Verifier writes that file.
